...
public class NotificationEventConsumer {
    ...
    public void onDriverRatingUpdated(Map<String, Object> event) {
        ...
        // ✅ Fix format specifiers: cast to Number
        double averageRating = ((Number) event.get("averageRating")).doubleValue();
        long totalReviews = ((Number) event.get("totalReviews")).longValue();

        notificationService.send(driverId, "⭐ Votre note a changé",
                String.format("Nouvelle moyenne : %.2f / 5 basée sur %d avis.", averageRating, totalReviews),
    ...
    // ==================== Bug #13: demande.merged consumer ====================
    ...
    public void onDemandeMerged(Map<String, Object> event) {
        String survivorId = (String) event.get("survivorDemandeId");
        Object mergedIdsObj = event.get("mergedDemandeIds");
        ...
        log.info("Demande merged event received: survivor={}, merged={}, passengers={}, route={}→{}",
                survivorId, mergedIdsObj, passengerIdsObj, departureCity, arrivalCity);

        // Notify all passengers whose demands were merged
        if (passengerIdsObj instanceof java.util.List<?> rawList) {
            List<String> passengerIds = rawList.stream()
                    .filter(o -> o instanceof String)
                    .map(Object::toString)
                    .toList();
            String title = "🔄 Votre demande a été fusionnée";
            String body = String.format("Votre demande collective %s → %s a été fusionnée avec une autre demande. Vous êtes toujours inscrit(e).",
                    departureCity != null ? departureCity : "?", arrivalCity != null ? arrivalCity : "?");
            for (String passengerId : passengerIds) {
                try {
                    notificationService.send(passengerId, title, body,
                            NotificationType.IN_APP, survivorId, "DEMANDE_MERGED", null);
                    log.info("Notified passenger {} about demande merge", passengerId);
                } catch (Exception e) {
                    log.error("Failed to notify passenger {}: {}", passengerId, e.getMessage());
                }
            }
        }
    }
