...
public class ReservationController {
    ...
    private void requireDriver(String role) {
        if (!"DRIVER".equalsIgnoreCase(role)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Only drivers can perform this action");
        }
    }
    ...
    public ResponseEntity<Boolean> internalCheckConfirmed(
        ...
        if (reservationId != null && !reservationId.isBlank()) {
            ...
            boolean match = r.getPassengerId().equals(passengerId)
                    && "CONFIRMED".equals(r.getStatus())
                    && (driverId == null || r.getDriverId().equals(driverId));
