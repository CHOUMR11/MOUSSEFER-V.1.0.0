...
public class AdminStatisticsController {
    ...
    @GetMapping
    @Operation(summary = "Unified dashboard statistics — aggregates users, reservations, payments, analytics")
    public ResponseEntity<Map<String, Object>> dashboard() {
        ...
        result.put("reservations", safe(() -> reservationServiceWebClient.get()
                .uri("/api/v1/reservations/internal/admin/stats")            // fixed
                .retrieve().bodyToMono(Map.class).block(TIMEOUT)));
        result.put("payments", safe(() -> paymentServiceWebClient.get()
                .uri("/api/v1/payments/internal/admin/stats")                // fixed
                .retrieve().bodyToMono(Map.class).block(TIMEOUT)));
